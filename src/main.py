...
def balloc(img):
    for i in range(0,1000,BPB):
        bb = img[(BB+i)*BSIZE+i:(BB+i+1)*BSIZE]
        for ii in range(0,1000-i):
            ...
            if (int.from_bytes(bbb,"little") & m) == 0:
                ii |= m
                a = 0
                img[(i+ii)*BSIZE:(i+ii+1)*BSIZE] = a.to_bytes(BSIZE,"little")
                return i + ii
    return 0
...
def daddent(img, dp, daddr,name,addr):
    ...
    """
    #print(img[int.from_bytes(img[addrs * BSIZE + j*SIZE_DE:addrs * BSIZE + j*SIZE_DE+2],"little"):int.from_bytes(img[addrs * BSIZE + j*SIZE_DE:addrs * BSIZE + j*SIZE_DE+2],"little") + 16])
    #exit()
    if name != ".":
...
def o_ls(img,args):
    ...
    if int.from_bytes(ip[:2],"little") == T_DIR:
        for i in range(0,int.from_bytes(ip[8:12],"little"),BSIZE):
            ...
            print(img[int.from_bytes(de[2*16:2*16+2],"little"):int.from_bytes(de[2*16:2*16+2],"little")+64])
            exit()
            """
...
def o_put(img,args):
    ...
    with open(inpath, "r") as f:
        s = f.read()
        if ip == None:
            addr,ip = icreat(img, root_inode,addr,fspath, T_FILE, None)
            if ip == None:
                return -1
        else:
